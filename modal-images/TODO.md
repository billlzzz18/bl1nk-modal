...
> Note: `search_service.py`, `deploy_bl1nk_search.py`, and search-related tests have moved to `modal-apps/bl1nk-search/`.

## Missing Tests
- [x] tests/test_build_rust.py - verify image build steps
- [x] tests/test_build_search.py - verify search image build
- [x] tests/test_search_service.py → moved to `modal-apps/bl1nk-search/tests/`
...
- [ ] search_service.py: `_index` and `_ids` are not thread-safe (global mutable state) → tracked in `modal-apps/bl1nk-search/`
- [x] pyproject.toml: `py-modules` updated (search modules moved to `modal-apps/bl1nk-search/pyproject.toml`)
