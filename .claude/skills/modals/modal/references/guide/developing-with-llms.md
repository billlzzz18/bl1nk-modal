...
If you're instead looking for information about how to run code generation agents *on Modal*,
...
- Reference documentation is available at: modal.com/docs/sdk/py/latest
...
- A group of Functions, Clses, and Sandboxes that are deployed together.
...
- Functions can be invoked in a number of ways. Some of the most common are:
  ...
  - `foo.spawn()` - Calls the Function with the given arguments, without waiting for the results. Terminating the App will also terminate spawned function calls.
- Web Functions: You can expose any Function to public HTTP traffic by adding a decorator:
...
- Volume: Provides a high-performance distributed file system for your Modal Apps.
...
- `modal serve modal_server.py` - Serve Web Function(s) associated with a Modal App, and hot-reload code on changes. Will print a URL to the Web Function(s). Note: you need to use `Ctrl+C` to interrupt `modal serve`.

### Deploying your Modal App

- `modal deploy path/to/your/app.py` - Deploy your App to Modal.
