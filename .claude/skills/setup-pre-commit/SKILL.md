...
- **Don't invent tooling a project doesn't have.** Check `pyproject.toml` dev deps before wiring a hook; as of this writing only `modal-sandbox` declares both `ruff` and `mypy` — the others declare `pytest` only.
